...
_TITA_XML_PATH = Path(__file__).parent.parent.parent.parent.parent.parent / "assets" / "robots" / "tita" / "robot.xml"


def _get_tita_spec() -> mujoco.MjSpec:
    """Load the TITA robot MuJoCo spec from XML."""
    return mujoco.MjSpec.from_file(str(_TITA_XML_PATH))


# Actuator groups — mirrors the IsaacLab ``DDT_TITA_CFG.actuators`` layout.
#
# IsaacLab DCMotorCfg for legs (non-wheel joints):
#   effort_limit=60.0, saturation_effort=80.0, velocity_limit=30.0,
#   stiffness=60.0, damping=1.5, friction=0.0
# → mjlab DcMotorActuatorCfg(stiffness=60.0, damping=1.5,
#   effort_limit=60.0, saturation_effort=80.0, velocity_limit=30.0)
#
# IsaacLab ImplicitActuatorCfg for wheels (leg_4 joints):
...
_TITA_LEG_ACTUATOR = DcMotorActuatorCfg(
    target_names_expr=(".*_leg_[123]",),
    stiffness=60.0,
    damping=1.5,
    effort_limit=60.0,
...
_TITA_WHEEL_ACTUATOR = IdealPdActuatorCfg(
    target_names_expr=(".*_leg_4",),
    stiffness=0.0,
    damping=0.5,
    effort_limit=12.0,
)

_TITA_SOFT_JOINT_POS_LIMIT_FACTOR = 0.9
_TITA_SOFT_VEL_LIMIT_FACTOR = 0.9
_TITA_SOFT_EFFORT_LIMIT_FACTOR = 0.9

# ---- regex patterns (ordered: more specific first) ----
# Joint space
LEG_JOINT_PATTERN = "joint_.*_leg_[123]"
WHEEL_JOINT_PATTERN = "joint_.*_leg_4"
ALL_JOINT_PATTERN = ".*"
LEG_1_JOINT_PATTERN = "joint_.*_leg_1"

# Actuator space (PD actuator names match the literal actuator names in robot.xml)
LEG_ACTUATOR_PATTERN = ".*_leg_[123]"
WHEEL_ACTUATOR_PATTERN = ".*_leg_4"
...
# Hard-coded TITA limits matching the XML spec.  These are no longer read
# by the cost functions from MJCF at runtime, which keeps training behaviour
# stable and explicit.
_TITA_JOINT_POS_LIMITS = {
    "joint_.*_leg_1": (-0.785398, 0.785398),
    "joint_.*_leg_2": (-1.91986, 3.49066),
    "joint_.*_leg_3": (-2.67035, -0.698132),
    WHEEL_JOINT_PATTERN: (-1.0e6, 1.0e6),
}

_TITA_JOINT_VEL_LIMITS = {
    LEG_JOINT_PATTERN: 30.0,
    WHEEL_JOINT_PATTERN: 30.0,
}

_TITA_ACTUATOR_EFFORT_LIMITS = {
    LEG_ACTUATOR_PATTERN: 60.0,
    WHEEL_ACTUATOR_PATTERN: 12.0,
}

_TITA_ARTICULATION = EntityArticulationInfoCfg(
    actuators=(_TITA_LEG_ACTUATOR, _TITA_WHEEL_ACTUATOR),
    soft_joint_pos_limit_factor=_TITA_SOFT_JOINT_POS_LIMIT_FACTOR,
)

_TITA_INIT_STATE = EntityCfg.InitialStateCfg(
    pos=(0.0, 0.0, 0.40),
    joint_pos=_TITA_DEFAULT_JOINT_POS,
    joint_vel={".*": 0.0},
)


def get_tita_cfg() -> EntityCfg:
    """Return a fresh ``EntityCfg`` for the TITA robot."""
    return EntityCfg(
        spec_fn=_get_tita_spec,
        articulation=_TITA_ARTICULATION,
        init_state=_TITA_INIT_STATE,
    )
...
def _tita_rough_cost_terms() -> dict:
    """Return cost terms for TITA NP3O constrained training.

    Four terms:
    - ``pos_limit``        — joint position limit violations (leg joints 1-3)
    - ``torque_limit``     — joint torque limit violations (leg actuators 1-3)
    - ``dof_vel_limits``   — joint velocity limit violations (leg joints 1-3)
    - ``default_joint``    — deviation from default pose (leg joints 1-3)
    """
    ...
    return {
        "pos_limit": CostTermCfg(
            ...
            params={
                ...
                "joint_pos_limit_patterns": _TITA_JOINT_POS_LIMITS,
                "soft_ratio": _TITA_SOFT_JOINT_POS_LIMIT_FACTOR,
            },
        ),
        "torque_limit": CostTermCfg(
            ...
            params={
                ...
                "actuator_effort_limit_patterns": _TITA_ACTUATOR_EFFORT_LIMITS,
                "soft_ratio": _TITA_SOFT_EFFORT_LIMIT_FACTOR,
            },
        ),
        "dof_vel_limits": CostTermCfg(
            ...
            params={
                "asset_cfg": SceneEntityCfg("robot", joint_names=LEG_JOINT_PATTERN),
                "joint_vel_limit_patterns": _TITA_JOINT_VEL_LIMITS,
                "soft_ratio": _TITA_SOFT_VEL_LIMIT_FACTOR,
            },
        ),
        "default_joint": CostTermCfg(
            ...
            params={
                "asset_cfg": SceneEntityCfg("robot", joint_names=LEG_JOINT_PATTERN),
                "default_joint_pos_patterns": _TITA_DEFAULT_JOINT_POS,
            },
        ),
    }
...
def _base_link_contact(
    ...
    # Find the body id for "base_link" — cached on first call.
    cache = getattr(env, "_tita_cache", None)
    if cache is None:
        cache = {}
        setattr(env, "_tita_cache", cache)
...
# ===========================================================================
# 4. TITA rough environment
# ===========================================================================


def tita_rough_env_cfg(
    num_envs: int = 4096,
    play: bool = False,
    flatten_policy_history: bool = False,
) -> ManagerBasedRlEnvCfg:
    """Build the TITA velocity-tracking environment configuration.
    ...
    priv_terms = {
        "contact_state": ObservationTermCfg(
            func=contact_state, params={"sensor_name": "contact_forces", "body_names": ".*_leg_4"},
    ...
    # ---- Actions ----
    # TITA is bipedal with 4 joints per leg: leg_1 (hip, pos), leg_2 (thigh, pos),
    # leg_3 (calf, pos), leg_4 (wheel, vel).  Mirrors DDT_TITA_CFG actuator layout.
    _action_scale = 0.25

    actions = {
        "left_leg_pos": JointPositionActionCfg(
            entity_name="robot",
            actuator_names=("joint_left_leg_1", "joint_left_leg_2", "joint_left_leg_3"),
            scale={
                "joint_left_leg_1": _action_scale,
                "joint_left_leg_2": _action_scale,
                "joint_left_leg_3": _action_scale,
            },
            clip={".*": (-100.0, 100.0)},
            preserve_order=True,
        ),
        "left_leg_vel": JointVelocityActionCfg(
            entity_name="robot",
            actuator_names=("joint_left_leg_4",),
            scale=5.0,
            clip={".*": (-100.0, 100.0)},
            preserve_order=True,
        ),
        "right_leg_pos": JointPositionActionCfg(
            entity_name="robot",
            actuator_names=("joint_right_leg_1", "joint_right_leg_2", "joint_right_leg_3"),
            scale={
                "joint_right_leg_1": _action_scale,
                "joint_right_leg_2": _action_scale,
                "joint_right_leg_3": _action_scale,
            },
            clip={".*": (-100.0, 100.0)},
            preserve_order=True,
        ),
        "right_leg_vel": JointVelocityActionCfg(
            entity_name="robot",
            actuator_names=("joint_right_leg_4",),
            scale=5.0,
            clip={".*": (-100.0, 100.0)},
            preserve_order=True,
        ),
    ...
    if not play:
        ...
        events["add_base_mass"] = EventTermCfg(
            ...
            params={
                ...
                "alpha_range": (-0.04, 0.06), "distribution": "uniform",
            },
        )
    ...
    rewards = {
        ...
        "base_height_l2": RewardTermCfg(func=base_height_l2, weight=-10.0, params={"target_height": 0.30}),
        ...
        "undesired_contacts": RewardTermCfg(
            ...
            params={"sensor_name": "contact_forces", "body_names": ".*_leg_3", "threshold": 1.0},
        ),
        "upward": RewardTermCfg(func=upward, weight=0.5),
        "default_joint_l2": RewardTermCfg(
            func=default_joint_l2, weight=-0.5,
            params={"asset_cfg": _LEG_JOINT_CFG, "default_joint_pos_patterns": _TITA_DEFAULT_JOINT_POS},
        ),
    }
    ...
    cfg.cost_terms = _tita_rough_cost_terms()

    return cfg


def tita_rough_play_env_cfg(
    num_envs: int = 50,
    flatten_policy_history: bool = False,
) -> ManagerBasedRlEnvCfg:
    """Convenience wrapper: TITA rough play config (noise/randomization disabled)."""
    return tita_rough_env_cfg(num_envs=num_envs, play=True, flatten_policy_history=flatten_policy_history)
